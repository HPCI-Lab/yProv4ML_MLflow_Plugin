...
# ---- Optional PROV exporter (call explicitly at run end) ----
try:
...
class CNN(nn.Module):
    def __init__(self, out_classes=10):
        ...
        self.sequential = nn.Sequential(
            nn.Conv2d(1, 32, 5),   # 28x28 -> 24x24x32
            nn.ReLU(),
            nn.Conv2d(32, 64, 5), # 24x24 -> 20x20x64
            nn.ReLU(),
        )
...
def _dataset(root: str, train: bool, tform):
    ...
    expected = "training.pt" if train else "test.pt"
    download = not os.path.exists(os.path.join(processed, expected))
...
class LRLogger:
    ...
    def log(self, step_type: str, index: int, optimizer):
        ...
        with open(self.path, "a", newline="") as f:
            csv.writer(f).writerow([step_type, index, lr])
        # also log to MLflow for easy charting
...
def main():
    p = argparse.ArgumentParser(description="MNIST + MLflow + yProv plugin demo")
    p.add_argument("--exp_name", type=str, default="mnist_yprov_plugin")
    p.add_argument("--run_name", type=str, default="mnist-hello-yprov")
    ...
    run_display_name = f"{args.run_name}-bs{args.batch_size}-lr{args.lr}-seed{args.seed}-sch{args.scheduler}"
    with mlflow.start_run(run_name=run_display_name) as run:
        ...
        # NEW: scheduler params (so you can filter/sort in UI)
        ...
        if TORCH_OK:
            ...
            train_ds = Subset(train_ds, range(args.batch_size * 200))  # quick demo subset
            ...
            for epoch in range(args.epochs):
                ...
                for i, (x, y) in enumerate(train_loader):
                    ...
                    if schedule_mode == "batch" and scheduler is not None:
                        scheduler.step()
                        lrlog.log("batch", global_step, optim)

                # epoch-level schedule (step/exp/cosine)
            ...
            # min/max lr handy for dashboards
        ...
        # --------- Always export PROV JSON for this run (via yprov plugin) ---------
        try:
